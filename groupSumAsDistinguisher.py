...
for power in range(start, end+1, increment):
	...
	for byteNum in range(0,16,4):
		...
		avgRanksOfGroups[grp] = sum([positions[i][1][grp] for i in range(0,numOfFiles)])/numOfFiles  # 1 represent the property sum
		#print sum([positions[i][3][grp] for i in range(0,numOfFiles)])/numOfFiles,	    
		varOfPositionOfGroups[grp] = np.var([positions[i][1][grp] for i in range(0,numOfFiles)])
